...
새 제품은 `products.json` 배열에 제품명(`name`), 상황 문구(`tagline`), 가격(`price`), 이미지 주소(`image`), 구매 링크(`url`)를 추가하면 됩니다. 전체 제품 페이지는 모든 항목을 표시하고, 홈은 10구 선물세트를 크게 소개한 뒤 다른 제품을 최대 5개 표시합니다.
